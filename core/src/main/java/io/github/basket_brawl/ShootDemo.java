...
public class ShootDemo implements Screen {
    ...
    private void renderBasketballShot() {
        ...
        float hoopX = camera.viewportWidth - 100f;
        float hoopY = camera.viewportHeight / 2f - 40f;
        spriteBatch.draw(hoopTexture, hoopX, hoopY, 80f, 80f);

        if (isTrajectoryActive) {
            float t = trajectoryTime / TRAJECTORY_DURATION;
            float ballX = ballStartX + (camera.viewportWidth - ballStartX - 50f) * t;
            float targetY = hoopY + 40f;
            ...
            if (shouldMakeShot) {
                ...
                ballY = ballStartY + (targetY - 150f - ballStartY) * t + MathUtils.sin(t * MathUtils.PI) * shotPeakHeight;
            }
    ...
    private void shootListener(float delta) {
        ...
        if (isHolding && canShoot == true) {
            ...
            ballStartY = camera.viewportHeight / 2f;
            ...
            shotPeakHeight = MathUtils.random(170f, 260f);
